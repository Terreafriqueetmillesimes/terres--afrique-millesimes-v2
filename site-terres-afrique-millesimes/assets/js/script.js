/* ======================================================================
   TERRES D'AFRIQUE & MILLÉSIMES — Script principal
   Menu mobile · header dynamique · animations reveal · panier ·
   billetterie QR · simulateur logistique export
   ====================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function majPanier() {
    ...
    if (!panier.length) panierBox.innerHTML =
      '<p style="color:var(--gris);font-size:.9rem">Votre panier est vide.</p>';
    if (panierTotal) panierTotal.innerHTML =
      `<span>Total</span><span>${total.toFixed(2)} €&nbsp;·&nbsp;
       ${Math.round(total * TAUX).toLocaleString('fr-FR')} FCFA</span>`;
  ...
  if (checkout) checkout.addEventListener('click', () => {
    ...
    alert('Redirection vers le paiement sécurisé Stripe / PayPal.\n' +
          '(Module à activer lors de la mise en production e-commerce.)');
  });

  /* ====================================================================
     SIMULATEUR LOGISTIQUE EXPORT AFRIQUE
     Paramètres alignés sur les Conditions Commerciales Export TAM
  ==================================================================== */
  const paramsExport = {
  ...
  if (simBtn) {
    simBtn.addEventListener('click', () => {
      ...
      if (cartons < 10) {
        out.innerHTML = '<p style="color:var(--or)">⚠ Quantité minimale export : 10 cartons mixtes (60 bouteilles).</p>';
        return;
      }
      const p = paramsExport[pays];
      ...
      out.innerHTML = `
        <table>
          <tr><td>Valeur marchandise HT</td><td style="text-align:right">${valeur.toFixed(2)} €</td></tr>
          <tr><td>Transport thermo-régulé (${bouteilles} btl)</td><td style="text-align:right">${transport.toFixed(2)} €</td></tr>
          <tr><td>Caisses isothermes (0,99 €/btl)</td><td style="text-align:right">${thermo.toFixed(2)} €</td></tr>
          <tr><td>Droits de douane estimés ${pays} (${p.douane} %)</td><td style="text-align:right">${douane.toFixed(2)} €</td></tr>
          <tr style="background:var(--noir-3)"><th>Coût rendu estimé</th>
            <th style="text-align:right">${total.toFixed(2)} € · ${Math.round(total*TAUX).toLocaleString('fr-FR')} FCFA</th></tr>
        </table>
        <p style="font-size:.78rem;color:var(--gris);margin-top:.6rem">
        Estimation indicative — Incoterm EXW Versailles. Droits de douane et taxes locales
        à la charge du client à destination. Devis ferme sur demande.</p>`;
    });
  }
  ...
  function majBon() {
    ...
    set('bon-cfa', Math.round((ht + tva) * TAUX).toLocaleString('fr-FR') + ' FCFA');
  }
